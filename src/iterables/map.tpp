...
F<B> funky::map(const std::function<B(A)> &mapper, const F<A> &source) {
    F<B> target;
...
template <template <typename...> typename F, typename A, typename B>
F<B> funky::map(const std::function<B(const A &)> &mapper, const F<A> &source) {
    F<B> target;
    std::transform(source.begin(), source.end(), std::back_inserter(target), mapper);
    return target;
}
...
std::vector<C> funky::map(const std::function<C(const A &, const B &)> &mapper, const std::map<A, B> &source) {
    std::vector<C> target;
